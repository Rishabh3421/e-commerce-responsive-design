...
/* <div class="product-card">
            <div src="" alt="" class="product-image"></div>
            <h2 class="product-name">Men Navy Blue Solid Sweatshirt</h4>
            <h2 class="product-brand">United Colors of Benetton</h5>
            <h2 class="product-rate">
                <span class="product-rate--currency">Rs</span> 2599 </h5>                
        </div>   */

let products_Clothing = document.querySelector("#products--clothing");
let products_Accessory = document.querySelector("#products--accessory");
let prodImage = document.querySelector(":root");


function addProduct(currProduct) {
    let prodCard = document.createElement("div");
    prodCard.setAttribute("class", "product-card");
    prodCard.setAttribute("id", `product-${productList.indexOf(currProduct)}`);
    let productImage = `<div class=product-image style=background-image:url(${currProduct.preview}) ></div>`;
    // prodImage.style.setProperty("--prod-image", `url(${currProduct.preview})`);
    let productName = `<h2 class=product-name>${currProduct.name}</h2>`;
    let productBrand = `<h2 class=product-brand>${currProduct.brand}</h2>`;
    let productRate = `<h2 class=product-rate><span class=product-rate-currency>Rs ${currProduct.price}</span></h2>`;
    prodCard.innerHTML = productImage + productName + productBrand + productRate;
    return prodCard;
}

productList.forEach(function(product) {
    if (product.isAccessory) {
        products_Accessory.appendChild(addProduct(product));
    } else {
        products_Clothing.appendChild(addProduct(product));
    }
});
...
function updateSliderPosition() {
    ...
    slider.style.transition = 'transform 0.5s ease-in-out'; // Added transition property
...
function startAnimation() {
    animationId = setInterval(autoSlide, 3000); // Changed to setInterval
}
